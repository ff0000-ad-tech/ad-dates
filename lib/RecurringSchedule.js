...
var RecurringSchedule = function ( dateModel ) {
	...
	*/
	...
	function parseModel() {
		...
		var labels = DateFormatter.getLabels('english').WEEKDAYS_FULL;
		...
		for ( var k = 0; k < tuneins.length; k++ ) {
			...
			for ( var q = 0; q < days.length; q++ ) {
				...
				date = DateFormatter.adjust( date, { day:dayShift });
				
				if ( DateManager.isPast( date, now ) ) {
					// now check if it is past the duration
					var temp = DateFormatter.adjust ( date, { minute:tuneinObj.eventDuration });
					if ( DateManager.isPast( temp, now ) ) {
						// if so, add a week to it
						date = DateFormatter.adjust( date, { day:7 });
					}					
				}
