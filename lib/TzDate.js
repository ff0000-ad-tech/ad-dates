...
function TzDate(args) {
	...
	if (Array.isArray(_dateString)) {
		...
		if (currentTzString) {
			...
			if (currentTzString == '+00:00') {
				_outputTimezone = Timezone.get('UTC') // Timezone.UTC
			// } else {
				// essentially this is a guess based on the value, perhaps it should just return the number?
				// let dls = Timezone.getDLS(new Date(_dateString))

				// let value = isoTzToNum(currentTzString, dls)
		...
		// 		let offset = Timezone.getDLS(new Date(_dateString))
	...
	if (args.outputTimezone) {
		log('if arg.outputTimezone', args.outputTimezone)
		_outputTimezone = Timezone.get(args.outputTimezone)
	} else if (adParams.defaultTimezone) {
		log('else if adParams.defaultTimezone')
		_outputTimezone = Timezone.get(adParams.defaultTimezone)
	}
	log('_outputTimezone:', _outputTimezone)
	log('_dateString:', _dateString)
	...
	*/
	T.getIn = function(inTimezone) {
		...
		log('   :::::::::::: utcString:', utcString, '|', T)

		// console.log('   :: Timezone._trueLOCAL:', Timezone._trueLOCAL)

		// var localTimezone = formatTimezoneISO(Timezone._trueLOCAL, 0)
		
		var localTimezone = Timezone.toISO(Timezone.get('local', T))
		log('   :::::::: localTimezone:', localTimezone)

		var tz = Timezone.get(inTimezone || T.outputTimezone, T)
		log('   ::::::::::::::::::: tz:', tz)

		let dateInLocal = utcString + localTimezone
		log('   :::::::::: dateInLocal:', dateInLocal)

		var utcDateAdjusted = new Date(utcString + localTimezone)
		log('   ::::: utcDateAdjusted:', utcDateAdjusted, 'in', tz.label)

		let adjustIn = DateUtils.adjust(utcDateAdjusted, { hour: tz.value })
		log('   :::::::::::: adjustIn:', adjustIn)
		return adjustIn
	}
