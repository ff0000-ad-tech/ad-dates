/**
	...
	@desc
		...
		this.schedule = new DateSchedule({
			...
			isStandard: true,
			standardOverrides: {
				DATE: date => date.format('${M}/${D} ${t} ${a^}'), // 8/01 8 PM
	...
	*/
	D.addDate = function(tzDate, label, callback) {
		...
		if (typeof label === 'function') {
			...
			const labelToPass = standardKey ? DateFormatter.getLabels()[standardKey] : null
	...
	*/
	...
	if (_targetDate && arg.isStandard == true) {
		...
		D.addDate(
			{ hour: -24 * 6 },
			function(date) {
				const dayNum = date.getIn(_targetDate.outputTimezone).getDay()
