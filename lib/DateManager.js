...
export default class DateManager {
	static defaultTimezone = undefined
	...
	*/
	static init = function(args = {}) {
		// TODO: pass in environment or create different check for INTERNAL-DATE
		console.log('DateManager.init()', args)

		if (_isInitialized) return
		_isInitialized = true

		let externalDate = getQueryParam('date')
		let dateMode = 'SYSTEM-DATE'

		if (externalDate) {
			_currentDate = externalDate
			dateMode = 'EXTERNAL-DATE'
		} else if (args.dateOverride && (args.environmentId == 'staging' || args.environmentId == 'debug')) {
		...
		if (_currentDate) {
			...
			let externalTzLabel = getQueryParam('tz')
			if (externalTzLabel) {
				tz = externalTzLabel
			}
		}
		...
		console.log('\t\t', this.getNow().toFullDateTime())
		console.log('')
		if (externalDefaultTimezone) {
	...
	/**
		...
		@desc
			...
			<codeblock>
				...
				var adParams = {
					dateSettings: {
						dateOverride : function() { return [ '2016-09-16 14:07:00', 'US/Eastern' ]; }
					}
				}
